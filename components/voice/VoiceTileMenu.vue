...
import { IconDots, IconScreenShare, IconScreenShareOff } from '~/components/icons/VoiceIcons'
...
<template>
  <div ref="menuEl" class="tile-menu-anchor">
    <Tooltip content="Ayarlar">
      <button class="tile-menu-btn" :class="{ 'is-visible': menuOpen || screenUnsubscribed }" @click="openMenu">
        <IconDots />
      </button>
    </Tooltip>
    ...
    <Transition name="menu-fade">
      <div v-if="menuOpen" class="tile-popover" @click.stop>
        ...
        <VolumeSlider
          :model-value="participantVolume"
          :label="mode === TileMode.ScreenShare ? 'Ekran Sesi' : 'Ses Seviyesi'"
          @update:model-value="onVolumeUpdate"
        />

        <button
          v-if="canToggleScreenShare"
          class="popover-action"
          @click="toggleScreenShareSubscription"
        >
