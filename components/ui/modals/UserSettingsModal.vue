...
import { getDisplayNameValidationError } from '@/src/utils/username'
...
const userHasChanges = computed(() => {
  const trimmedName = displayNameValue.value.trim()
  const trimmedAvatar = selectedAvatarSeed.value.trim()
  return trimmedName !== currentDisplayName.value || trimmedAvatar !== currentAvatarSeed.value
})
...
async function onCameraQualityChange(event: Event) {
  ...
  if (!['720p60', '1080p30'].includes(value)) return
...
function initializeModal() {
  ...
  displayNameValue.value = app.displayName ?? ''
...
async function onSaveProfile() {
  ...
  const opts: { display_name?: string; avatar_seed?: string } = {}
  resetProfileFeedback()

  if (trimmedName !== currentName) {
    const displayNameError = getDisplayNameValidationError(trimmedName)
    if (displayNameError) {
      usernameError.value = displayNameError
      return
    }
    opts.display_name = trimmedName
  }
  ...
  if (!opts.display_name && !opts.avatar_seed) return

  app.clearCommandError(userUpdateType.value)
  await socket.updateUserProfile(opts)
}
...
async function onSubmitChangedUsername(nextUsername: string) {
  // Username/account handle change only; does not touch the visible display name.
  changeUsernameOpen.value = false
  app.clearCommandError(userUpdateType.value)
  await socket.updateUserProfile({ username: nextUsername })
}
...
watch(
  ...
  () => {
    ...
    formError.value = updateErr ? updateErr.message : ''
  },
  { deep: true }
)
...
<template>
  <Modal
    ...
    :dismissible="!deleteAccountLoading"
    ...
    <div class="settings-body">
      <template v-if="tab === SettingsTab.Profile">
        <div class="field">
          ...
          <input id="profile-display-name" v-model="displayNameValue" class="input" type="text" />
        </div>
      ...
      <template v-if="tab === SettingsTab.Voice">
        ...
        <div class="field">
          ...
          <div class="nc-method-row">
            ...
            >
              <option :value="NoiseCancellationMethod.WebRTC">WebRTC (Tarayıcı)</option>
              <option :value="NoiseCancellationMethod.Krisp" disabled>Krisp (Yakında)</option>
            </select>
            <p class="nc-method-hint">Krisp yakında kullanılabilir olacak.</p>
          </div>
