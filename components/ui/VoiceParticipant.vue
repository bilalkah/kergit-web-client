...
import { IconVolumeBoosted, IconVolumeLow, IconVolumeMuted, IconMicMuted, IconHeadphonesOff, IconScreenShare } from '~/components/icons/VoiceIcons'
...
const isScreenSharing = computed(() =>
  props.participant.isScreenSharing === true
  || app.screenSharingUsersById[props.participant.userId] === true
)
...
<template>
  <button class="ui-voice-participant" type="button" @click="onClick" @contextmenu="onContextMenu"
    @touchstart.passive="onTouchStart" @touchend="onTouchEnd" @touchmove="onTouchMove" @touchcancel="onTouchCancel">
    <Avatar class="ui-voice-avatar" :src="userAvatarUrl(participant.avatarSeed)" :alt="participant.displayName"
      :size="18" :speaking="participant.speaking === true" />
    <span class="ui-voice-name">{{ participant.displayName }}</span>

    <div v-if="volumeIndicator || isScreenSharing || voiceState.muted || voiceState.deafened"
      class="ui-voice-state-icons">
      <Tooltip v-if="volumeIndicator" :content="`Kullanıcı ses seviyesi ${volumeOverride}%`">
        <span class="ui-voice-state-icon volume" :class="volumeIndicator"
      ...
      <Tooltip v-if="isScreenSharing" content="Ekran paylaşıyor">
        <span class="ui-voice-state-icon screen-sharing" aria-label="Ekran paylaşıyor">
          <IconScreenShare :size="14" />
        </span>
      </Tooltip>

      <Tooltip v-if="voiceState.muted" content="Mikrofon kapalı">
        <span class="ui-voice-state-icon" :class="{ muted: voiceState.muted }" aria-label="Mikrofon kapalı">
          <IconMicMuted :size="14" />
        </span>
      </Tooltip>

      <Tooltip v-if="voiceState.deafened" content="Kulaklık kapalı">
        <span class="ui-voice-state-icon" :class="{ muted: voiceState.deafened }" aria-label="Kulaklık kapalı">
...
.ui-voice-state-icon.screen-sharing {
  color: #22c55e;
}
</style>
