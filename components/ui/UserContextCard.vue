...
const showVolumeControl = computed(() => {
  ...
  if (props.userId === app.userId) return false
  if (!app.hasLocalActiveVoice || app.activeVoiceChannelId !== props.channelId) return false
...
const hasModerationActions = computed(() =>
  isVoiceMode.value && (
    canPromoteToAdmin.value ||
    canDemoteToMember.value ||
    canKickFromVoice.value ||
    ...
    <div ref="cardEl" class="ui-profile-card" :style="positionStyle" role="dialog" aria-modal="false">
      ...
      <div class="ui-profile-body">
        ...
        <div class="ui-profile-role-row">
          <span class="ui-role-badge" :class="`is-${targetRoleMeta.key}`">{{ targetRoleMeta.badge }}</span>
          <span class="ui-profile-role-label">{{ targetRoleMeta.label }}</span>
        </div>
        <div class="ui-profile-common">🔗 {{ commonHubs }} Ortak Sunucu</div>
      </div>
      ...
      <div v-if="showVolumeControl" class="ui-profile-volume">
        ...
        <div class="ui-profile-volume-row">
          <input
            class="ui-profile-volume-slider"
            type="range"
            min="0"
            max="200"
            step="1"
      ...
      <div v-if="isVoiceMode && !showVolumeControl" class="ui-profile-volume ui-profile-volume--disabled">
        <div class="ui-profile-volume-title">KULLANICI SES SEVİYESİ</div>
        <div class="ui-profile-volume-hint">Ses kontrolü için aynı ses kanalında olmalısın.</div>
      </div>

      <div v-if="hasModerationActions" class="ui-profile-admin">
        <div class="ui-profile-admin-title">MODERASYON</div>
        ...
        <button v-if="canKickFromVoice" class="ui-profile-action" type="button" @click="onKickFromVoice">
          Sesten At
        </button>
