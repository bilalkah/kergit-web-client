...
const CHANNEL_NAME_MAX_LENGTH = 80 // matches kergit_app.channels name constraint
...
const voiceParticipantsForChannel = (channelId: string) => {
    ...
    return ids
        .map(id => {
            ...
            const isScreenSharing = app.screenSharingUsersById[id] === true
            return {
                userId: id,
...
async function onCreateChannel() {
    ...
    if ([...trimmedName].length > CHANNEL_NAME_MAX_LENGTH) {
        createChannelError.value = `Kanal adı en fazla ${CHANNEL_NAME_MAX_LENGTH} karakter olabilir.`
        return
    }
...
async function onSaveChannelRename() {
    ...
    if ([...trimmedName].length > CHANNEL_NAME_MAX_LENGTH) {
        channelRenameError.value = `Kanal adı en fazla ${CHANNEL_NAME_MAX_LENGTH} karakter olabilir.`
        return
    }
...
<template>
    <div class="channel-col" v-bind="attrs">
        <HubHeader v-if="app.viewedHub" :mobile-mode="mobileMode" @invite-people="openInvite"
            @create-channel="openCreateChannel" @hub-settings="openSettings" @leave-hub="onLeaveHub"
            @close="emit('close')" />
        ...
        <div class="channel-list" v-else-if="app.viewedChannels.length">
            <div class="channel-section">
                <SidebarSectionHeader title="METİN KANALLARI" :collapsed="textSectionCollapsed"
                    :show-add-icon="canManage" add-icon-label="Kanal oluştur" @toggle="toggleTextSection"
                    @create="openCreateChannel" />
                <Transition name="section-collapse">
                    <div v-show="!textSectionCollapsed" class="channel-section-items">
                        <ChannelItem v-for="ch in textChannels" :key="ch.id" class="channel group"
                            :active="ch.id === app.activeChannelId" :show-settings="canManage"
                            settings-aria-label="Kanal ayarları" @click="openChannel(ch.id)"
            ...
            <div class="channel-section">
                <SidebarSectionHeader title="SESLİ KANALLAR" :collapsed="voiceSectionCollapsed"
                    :show-add-icon="canManage" add-icon-label="Kanal oluştur" @toggle="toggleVoiceSection"
                    @create="openCreateChannel" />
                <Transition name="section-collapse">
                    <div v-show="!voiceSectionCollapsed" class="channel-section-items">
                        <div v-for="ch in voiceChannels" :key="ch.id" class="voice-channel-block">
                            <ChannelItem class="channel group" :channel-type="ChannelType.Voice" :channel-name="ch.name"
                                ...
                                :show-settings="canManage" settings-aria-label="Kanal ayarları"
                                @click="openChannel(ch.id)" @settings="onChannelSettings(ch)"
                                @show-profile="onShowVoiceControls(ch.id, $event)"
        ...
        <div class="channel-empty" v-else>
            <EmptyState icon="📭" :title="emptyStateTitle" :message="emptyStateMessage"
                :action-text="emptyStateActionText" compact @action="openCreateChannel" />
        </div>

    </div>

    <ProfileCard v-if="activeUserCard && activeUserCard.mode === 'profile'" :user-id="activeUserCard.userId"
        :position="{ x: activeUserCard.x, y: activeUserCard.y }" :hub-id="app.viewedHubId" @close="closeProfile"
        @update-role="onUpdateRole" @kick-user="onKickUser" @kick-user-from-voice="onKickUserFromVoice" />

    <VoiceParticipantCard v-if="activeUserCard && activeUserCard.mode === 'voice-controls'"
        :user-id="activeUserCard.userId" :position="{ x: activeUserCard.x, y: activeUserCard.y }"
        :hub-id="app.viewedHubId" :channel-id="activeUserCard.voiceChannelId ?? null" @close="closeProfile"
        @update-role="onUpdateRole" @kick-user="onKickUser" @kick-user-from-voice="onKickUserFromVoice"
                ...
                <div class="hub-content">
                    <div class="hub-field">
                        ...
                        <div class="hub-code-row">
                            ...
                            <button class="hub-btn primary" @click="copyInvite"
    ...
    <HubSettingsModal v-model="settingsOpen" :hub-name="renameValue" :selected-avatar-seed="selectedAvatarSeed"
        :avatar-seeds="avatarSeeds" :has-changes="hubHasChanges" :can-manage="canSettings" :can-delete="canDeleteHub"
        :rename-error="renameError" :avatar-error="avatarError" :delete-confirm="deleteConfirm"
        @update:hub-name="renameValue = $event" @update:selected-avatar-seed="selectedAvatarSeed = $event"
        @save="onSaveHubSettings" @delete="onDeleteHub" />

    <CreateChannelModal v-model="createChannelOpen" :channel-type="createChannelType" :channel-name="createChannelName"
        :can-create="canSettings" :submitting="createChannelSubmitting" :error="createChannelError"
        @update:channel-type="createChannelType = $event" @update:channel-name="createChannelName = $event"
        @create="onCreateChannel" />

    <ChannelSettingsModal v-model="channelSettingsOpen" :channel-name="channelRenameValue"
        :has-changes="channelHasChanges" :can-manage="canManage" :can-delete="canDeleteChannel"
        :rename-error="channelRenameError" :delete-error="channelDeleteError" :delete-confirm="channelDeleteConfirm"
        @update:channel-name="channelRenameValue = $event" @save="onSaveChannelRename" @delete="onDeleteChannel" />
                ...
                <div class="hub-header-row">
                    <div>
                        ...
                        <div class="hub-sub">
                            ...
                            Ayrılıp <strong>{{ pendingVoiceSwitch?.channelName }}</strong> kanalına geçmek istiyor
                            musun?
                        </div>
                    </div>
