...
export interface UserInfo { id: string; username: string; display_name: string; avatar_seed: string }
...
export interface ChatMessage {
    ...
    // Channel-local monotonic sequence; 0 until the message is persisted.
    messageSeq: number
...
export interface MessageAppendPatch {
    ...
    message: {
        ...
        author?: { id: string; metadata?: { username?: string; avatar_seed?: string; display_name?: string } }
    }
}
...
export interface HubMemberUpsertPatch {
    ...
    user?: { id: string; metadata?: { username?: string; avatar_seed?: string; display_name?: string } }
}
...
// Krisp AI noise cancellation is temporarily disabled. While this is false the
// method is never read or persisted as Krisp; any legacy stored 'krisp' value is
// normalized back to WebRTC on read.
const KRISP_ENABLED = false

function readNoiseCancellationMethod(): NoiseCancellationMethod {
    if (!KRISP_ENABLED) {
        // Overwrite any legacy 'krisp' value so it does not linger in storage.
        if (canUseLocalStorage()) {
            try {
                if (window.localStorage.getItem(VOICE_NOISE_CANCELLATION_METHOD_STORAGE_KEY) === NoiseCancellationMethod.Krisp) {
                    window.localStorage.setItem(VOICE_NOISE_CANCELLATION_METHOD_STORAGE_KEY, NoiseCancellationMethod.WebRTC)
                }
            } catch {
                // Ignore storage failures.
            }
...
function writeNoiseCancellationMethod(method: NoiseCancellationMethod) {
    if (!canUseLocalStorage()) return
    // Never persist Krisp while it is disabled.
    const persistedMethod = !KRISP_ENABLED ? NoiseCancellationMethod.WebRTC : method
    try {
        window.localStorage.setItem(VOICE_NOISE_CANCELLATION_METHOD_STORAGE_KEY, persistedMethod)
    } catch {
        // Ignore storage failures.
    }
}
...
export const useAppStore = defineStore('app', () => {
    ...
    const screenSharingUsersById = ref<Record<string, boolean>>({})
    ...
    function setUserScreenSharing(userIdInput: string | null | undefined, sharing: boolean) {
        const normalizedUserId = normalizeId(userIdInput)
        if (!normalizedUserId) return

        const next = { ...screenSharingUsersById.value }
        ...
        screenSharingUsersById.value = next
    }

    function clearScreenSharingUsers() {
        screenSharingUsersById.value = {}
    }
    ...
    function getUserDisplayName(hubId: string | null | undefined, targetUserId: string | null | undefined): string {
        ...
        // Visible name is display_name; fall back to the @handle only if it is missing.
        ...
        return attachments
            .map((attachment) => {
                ...
                return {
                    ...
                    storageBucket: normalizeId(attachment.storage_bucket),
                    storageKey,
    ...
    const normalizeMessage = (
        ...
        const messageSeqRaw = raw.message_seq
        const messageSeqParsed = typeof messageSeqRaw === 'number' ? messageSeqRaw : Number(messageSeqRaw ?? 0)
        return {
            ...
            messageSeq: Number.isFinite(messageSeqParsed) && messageSeqParsed > 0 ? Math.floor(messageSeqParsed) : 0,
    ...
    const rebuildVoiceDerivedState = () => {
        ...
        const nextScreenSharingUsers: Record<string, boolean> = {}
        for (const sharingUserId of Object.keys(screenSharingUsersById.value)) {
            if (nextStatesByUser[sharingUserId]) {
                nextScreenSharingUsers[sharingUserId] = true
            }
        }
        ...
        screenSharingUsersById.value = nextScreenSharingUsers
    ...
    const upsertMessages = (
        ...
        next.sort((a, b) => {
            // Channel-local message_seq is the authoritative order. Fall back to
            // created_at only for messages not yet persisted (messageSeq === 0).
            const seqA = a.messageSeq || 0
            const seqB = b.messageSeq || 0
    ...
    function hydrateFromStateSync(payload: StateSyncPayload) {
        ...
        const selfDisplayName = payload.self?.metadata?.display_name ?? ''
        const selfAvatar = payload.self?.metadata?.avatar_seed ?? ''

        // identity
        userId.value = selfId || null
        username.value = selfName
        displayName.value = selfDisplayName || selfName
            ...
            for (const userOp of userOps) {
                ...
                if (user) {
                    ...
                    if (userIdValue === normalizeId(userId.value)) {
                        ...
                        const nextDisplayName = user.metadata?.display_name ?? ''
                    ...
                    if (appendMessage) {
                        appendMessagePatch({
                            ...
                            message: {
                                ...
                                message_seq: appendMessage.message_seq ?? 0,
    ...
    function showVoiceGrid() {
        // Opening the grid must NOT auto-subscribe to remote camera videos.
        // Remote camera is opt-in per participant via the voice grid tiles.
    ...
    function hideVoiceGrid() {
        ...
        if (voiceConnectedByThisSession.value) {
            // Data-saving cleanup: drop all remote camera opt-ins and unsubscribe.
    ...
    function setNoiseCancellationMethod(method: NoiseCancellationMethod) {
        const nextMethod = (KRISP_ENABLED && method === NoiseCancellationMethod.Krisp)
    ...
    function getLatestMessageCursor(channelId: string): MessageCursor | null {
        ...
        // Use the highest persisted sequence; a just-sent message may still have
        // messageSeq === 0 until it is reloaded from the server.
        let maxSeq = 0
        for (const message of list) {
            if (message.messageSeq > maxSeq) maxSeq = message.messageSeq
        }
        if (maxSeq <= 0) return null
        return { message_seq: maxSeq }
    }
    ...
    function getOldestMessageCursor(channelId: string): MessageCursor | null {
        ...
        if (!list || list.length === 0) return null
        const oldest = list[0]
        if (!oldest || oldest.messageSeq <= 0) return null
        return { message_seq: oldest.messageSeq }
    }
    ...
    function upsertHubMemberPatch(event: HubMemberUpsertPatch) {
        ...
        if (event.user) {
            ...
            hubUsers[memberId] = {
                ...
                display_name: event.user.metadata?.display_name ?? '',
