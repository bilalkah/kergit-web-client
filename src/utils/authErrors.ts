...
function mapSignupMessage(
  ...
  // Supabase Auth: the email is already registered.
  ...
  // The profile-creation trigger hit the unique username index. Supabase surfaces
  // this as a 500 "Database error saving new user"; it can also appear as a race
  // after the username precheck passed.
  if (containsAny(normalizedMessage, [
    'database error saving new user',
    'idx_profiles_user_name_unique',
    'profiles_user_name',
    'duplicate key value',
    'unique constraint',
    'unique_violation',
    ...
    return 'Bu kullanıcı adı kullanımda. Lütfen başka bir kullanıcı adı seçin.'
  }
