...
export const protoService = {
  ...
  encodeSelectActiveChannel(
    ...
    channelId: string,
    latestLimit: number,
    knownLatestCursor?: { message_id: string; created_at_unix_us: string | number }
  ): Uint8Array {
    return SelectActiveChannel.encode(
      SelectActiveChannel.create({
        ...
        known_latest_cursor:
          knownLatestCursor && knownLatestCursor.message_id.length > 0
            ? {
                message_id: knownLatestCursor.message_id,
                created_at_unix_us: knownLatestCursor.created_at_unix_us,
              }
            : undefined,
      })
    ).finish()
  ...
  encodeFetchMessagesBefore(
    hubId: string,
    channelId: string,
    beforeCursor: { message_id: string; created_at_unix_us: string | number },
    limit: number
  ): Uint8Array {
    return FetchMessagesBefore.encode(
      FetchMessagesBefore.create({
        channel: channelRef(hubId, channelId),
        before_cursor: {
          message_id: beforeCursor.message_id,
          created_at_unix_us: beforeCursor.created_at_unix_us,
        },
        limit,
      })
    ).finish()
  ...
  encodeUpdateUser(opts: { username?: string; avatar_seed?: string }): Uint8Array {
