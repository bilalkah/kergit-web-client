...
// runtime never loads/applies @livekit/krisp-noise-filter and always falls back
// to browser WebRTC noise suppression, regardless of the requested method.
export const KRISP_ENABLED = false;
...
const AUTO_THRESHOLD_OFFSET = 3;
const AUTO_THRESHOLD_MIN = 5;
...
export function createVoiceInputHandler(options: VoiceInputHandlerOptions): VoiceInputHandler {
    ...
    async function applyNoiseProcessorOrFallback(
        ...
        const krispApplied = await applyKrispToTrack(targetTrack);
    ...
    async function restartTrackWithVoiceProcessing(
        ...
        const useKrisp = KRISP_ENABLED && settings.noiseCancellationMethod === NoiseCancellationMethod.Krisp;
        ...
        for (const deviceId of candidateDeviceIds) {
            try {
                ...
                await applyNoiseProcessorOrFallback(targetTrack, useKrisp, deviceId);
                return;
            } catch (err) {
        ...
        await applyNoiseProcessorOrFallback(targetTrack, useKrisp);
    }
