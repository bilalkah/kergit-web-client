...
import { createVoiceInputHandler, NoiseCancellationMethod } from "@/src/services/webrtc/inputHandler";
...
import { applyKrispToTrack, destroyKrisp, removeKrispFromTrack } from "@/src/services/webrtc/krispProcessor";
...
type VoiceConnectOptions = {
    ...
    onPermanentDisconnect: () => void | Promise<void>;
...
function getVideoMode(): VideoMode {
    try {
        return useAppStore().videoMode === VideoMode.Quality
            ? VideoMode.Quality
            : VideoMode.Performance;
    } catch {
        return VideoMode.Performance;
    }
}
...
function shouldSubscribeToRemotePublication(
    ...
    if (publication.kind === Track.Kind.Video) {
        ...
        try {
            return useAppStore().viewingVoiceGrid;
        } catch {
            return false;
        }
    }
...
async function connectOnce(sessionId: number, token: string, options: VoiceConnectOptions) {
    ...
    try {
        ...
        const useKrisp = voiceProcessingSettings.noiseCancellationMethod === NoiseCancellationMethod.Krisp;
...
function cameraSimulcastLayers(preset: VideoQualityPreset) {
    if (preset === '1440p30') return [VideoPresets.h360, VideoPresets.h1080];
    if (preset === '1080p30' || preset === '1080p60') return [VideoPresets.h360, VideoPresets.h720];
...
function screenShareEncoding(preset: VideoQualityPreset, mode: VideoMode) {
    if (preset === '1440p30') {
        return {
            maxBitrate: mode === VideoMode.Quality ? 7_000_000 : 5_000_000,
            maxFramerate: 30,
        };
    }
    if (preset === '1080p60') {
        return {
            maxBitrate: mode === VideoMode.Quality ? 6_000_000 : 4_500_000,
            maxFramerate: 60,
        };
    }
    if (preset === '1080p30') {
    ...
    if (preset === '720p60') {
        return {
            maxBitrate: mode === VideoMode.Quality ? 3_500_000 : 2_500_000,
            maxFramerate: 60,
        };
    }
...
function screenShareSimulcastLayers(
    preset: VideoQualityPreset,
    mode: VideoMode,
) {
    if (preset === '1440p30') {
        return limitSimulcastLayers(
            mode === VideoMode.Quality
                ? [VideoPresets.h360, VideoPresets.h1080]
                : [VideoPresets.h360, VideoPresets.h720],
            "screen",
        );
    }
    if (preset === '1080p30' || preset === '1080p60') {
        return limitSimulcastLayers(
            mode === VideoMode.Quality
                ? [VideoPresets.h360, VideoPresets.h720]
...
export async function enableScreenShare() {
    ...
    const encoding = screenShareEncoding(preset, mode);
    const layers = screenShareSimulcastLayers(preset, mode);
...
/**
 * Subscribe or unsubscribe from remote camera tracks.
 * Screen-share tracks remain opt-in and are not toggled here.
 * Audio subscriptions are left untouched.
 * Used to save bandwidth when the voice grid is not visible.
 */
    ...
    room.remoteParticipants.forEach((participant) => {
        participant.trackPublications.forEach((publication) => {
            ...
            try {
                ...
                devWarn("[voice] remote video subscription toggle failed", err);
            }
        });
    });
    devLog(`[voice] remote video subscriptions ${subscribed ? "enabled" : "disabled"}`);
}

/** Disable screen sharing and unpublish video + audio screen tracks. */
export async function disableScreenShare() {
    if (!state.screenTrack && !state.screenAudioTrack) return;
